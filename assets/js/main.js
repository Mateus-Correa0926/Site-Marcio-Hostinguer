...
(function () {
  ...
  for (var c = 0; c < contactForms.length; c++) {
    contactForms[c].addEventListener('submit', function (e) {
      e.preventDefault();
      var form = this;
      var formData = new FormData(form);

      var payload = mapContactPayload(formData);
  ...
  function mapContactPayload(formData) {
    var type = (formData.get('type') || 'couple').toLowerCase();
    var firstName = (formData.get('first_name') || '').trim();
    var lastName = (formData.get('last_name') || '').trim();
    var plannerName = (formData.get('name') || '').trim();
    var fullName = firstName || lastName ? (firstName + ' ' + lastName).trim() : plannerName;
    ...
    var payload = {
      ...
      event_date: (formData.get('wedding_date') || '').trim() || null,
      event_location: (formData.get('location') || '').trim(),
